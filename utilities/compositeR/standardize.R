...
standardizeOverRandomInterval <- function(ages,pdm,duration,searchRange,normalizeVariance = TRUE,minN = 8){
  ...
  for(i in 1:ncol(pdm)){
    ...
    goodAges <-   ages[!is.na(pdm[,i])] #for that column: where there are ages

    pStart <- min(goodAges) #max and min of data for that record
    pEnd <- max(goodAges)

    if(pEnd-pStart < duration){ #check. is the length of that binned record longer than the duration I specified in the inputs to the function?
    ...
    if(length(startOptions) < 1){
      warning(paste("column",i,"doesnt have an overlap between the required duration and the searchRange")) #this might happen if your duration is too long, or if the record only has a few very young data points
      scaledPdm[,i] <- NA
      next
    }
    ...
    nVal <- 0
    tt <- 0
    posStarts <- sample(startOptions) # shuffles the possible start positions
    for(ps in posStarts){ # for each of the (shuffled) starting options
      iStart <- ps
      iEnd <- iStart+duration #effectively this means that we're standardising the record in sequential chunks, from every possible start point

      #check nvalues
      pass <- which(ages>=iStart & ages<iEnd) # collect only the ages that are within the chunk we're now looking at
      nVal <- sum(is.finite(removeConsecutiveDuplicates(pdm[pass,i])))

      if(nVal >= minN){ #check for number of values within that little chunk. Chunk is of length 'duration', and starts randomly in a place where it is possible to do so. Keep checking until we find a place that works! This place is the winner, and ends up being the chunk of record who's mean is used for the standardisation. 
        break #move on
      }
    }
...
standardizeMeanIteratively <- function(ages,pdm,duration,searchRange,normalizeVariance = TRUE,thresh = 0.01,minN = 8){
  ...
  while(delta > thresh){
    ...
    for(j in rvec){
      ...
      es <- recordRMSE(upmat[,j],upmat) # difference matrix between the column of interest, and the rest of the scaled data matrix. then subtract the bias. 
